...
osg::Group* SolarInteractiveHandler::createSVFCameras()
{
...
RenderSurface* SolarInteractiveHandler::cubemap2hemispherical(osg::Group * svfCameraBuffers)
{
...
double SolarInteractiveHandler::calSVF(osg::Image * img, bool applyLambert)
{
...
SolarRadiation SolarInteractiveHandler::calSolar(osg::Image* img, SolarParam* solarParam, osg::Vec3d pos, osg::Vec3d normal, osg::Node* sceneNode)
{
	...
	OnResultsUpdated resultsCallback)
{
	...
	_cubemapCameras = createSVFCameras();
	_cubemapCameras->setNodeMask(false);
	root->addChild(_cubemapCameras);

	for (size_t i = 0; i < _cubemapCameras->getNumChildren(); i++)
	{
		((CubemapSurface*)_cubemapCameras->getChild(i))->setRenderOrder(osg::Camera::PRE_RENDER, i);
		((CubemapSurface*)_cubemapCameras->getChild(i))->addChild(sceneNode);
	}
  ...
  //_cubemap2fisheyeCamera = CameraBuffer::createSlave(512, 512, contexts[0]);
	_cubemap2fisheyeCamera = cubemap2hemispherical(_cubemapCameras);
...
//	_manip->setByInverseMatrix(viewmat);
	...
	if (_cubemap2fisheyeCamera)
	{
		 ...
		 double svf = calSVF(_cubemap2fisheyeCamera->Image().get(), false);
...
{
	auto cubemapCameras = getCubemapCameras(_cubemapCameras);
